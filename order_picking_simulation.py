...
class Bank_Simulation:
    ...
    def time_adv(self):                                                       #timing routine
        print("self.t_arrival",self.t_arrival)  
        print("self.t_departure1",self.t_departure1)
        print("self.t_departure2",self.t_departure2)
        t_next_event=min(self.t_arrival,self.t_departure1,self.t_departure2)  #determine time of next event
        print("before total time wait: ",self.total_wait_time)
        self.total_wait_time += (self.num_in_q*(t_next_event-self.clock))
        print("after total time wait: ",self.total_wait_time)
    ...
    def gen_int_arr(self):                                             #function to generate arrival times using inverse transform                                          
        return (-np.log(1-(np.random.uniform(low=0.0,high=1.0))) * 3)
    
    def gen_service_time_teller1(self):                                #function to generate service time for teller 1 using inverse transform
        return (-np.log(1-(np.random.uniform(low=0.0,high=1.0))) * 1.2)
    
    def gen_service_time_teller2(self):                                #function to generate service time for teller 2 using inverse transform
...
print(df.head())
df.to_csv('picking_simulation.csv',index=False)    
